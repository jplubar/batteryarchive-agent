...
def get_cell_status(cell_id, engine):
    cell_metadata_table = Table("cell_metadata", metadata_obj, autoload_with=engine)
...
def get_module_status(module_id, engine):
    module_metadata_table = Table("module_metadata", metadata_obj, autoload_with=engine)
    stmt = select(module_metadata_table.c.status).where(module_metadata_table.c.module_id==module_id)
    with engine.connect() as conn:
        result = conn.execute(stmt).first()
        if result is not None:
            return result[0]
...
def set_module_status(module_id, status, engine):
    module_metadata_table = Table("module_metadata", metadata_obj, autoload_with=engine)
    stmt = update(module_metadata_table).where(module_metadata_table.c.module_id==module_id).values(status=status)
    with engine.connect() as conn:
        result = conn.execute(stmt)
        conn.commit()


def import_module_metadata(df_module_metadata: pd.DataFrame, module_id: str, engine):
    module_metadata_table = Table("module_metadata", metadata_obj, autoload_with=engine)
    module_status = get_module_status(module_id, engine)
    if module_status is not None:
        return
    series_module_metadata = df_module_metadata[df_module_metadata["module_id"] == module_id].iloc[0]
    with engine.connect() as conn:
        stmt = insert(module_metadata_table).values(
            module_id=module_id,
            configuration=series_module_metadata["Configuration"],
            num_parallel=int(series_module_metadata["# cells in parallel"]),
            num_series=int(series_module_metadata["# cell in series"]),
            status="buffering"
        )
        result = conn.execute(stmt)
        conn.commit()
...
def import_module_cell_metadata(df_configuration: pd.DataFrame, df_module_metadata: pd.DataFrame, module_id: str, engine):
    cell_metadata_table = Table("cell_metadata", metadata_obj, autoload_with=engine)
    cycle_metadata_table = Table("cycle_metadata", metadata_obj, autoload_with=engine)
    series_module_metadata = df_module_metadata[df_module_metadata["module_id"] == module_id].iloc[0]
    for index, row in df_configuration.iterrows():
        if row["Type"] == "Module":
            # Modules are not added to cell metadata tables
            continue
        cell_id = "{}_{}".format(series_module_metadata["module_id"], row["Name"])
        ...
        if current_cell_status is None:
            with engine.connect() as conn:
                stmt = insert(cell_metadata_table).values(
                    cell_id=cell_id,
                    cathode=series_module_metadata["cathode"],
                    anode=series_module_metadata["anode"],
                    ah=float(series_module_metadata["ah"]),
                    form_factor=str(series_module_metadata["form_factor"]),
                    test=series_module_metadata["test"],
                    tester=series_module_metadata["tester"],
                    source=series_module_metadata["source"],
                    parent_id=module_id,
                    status="buffering"
                )
                result = conn.execute(stmt)
                stmt = insert(cycle_metadata_table).values(
                    cell_id=cell_id,
                    temperature=float(series_module_metadata["temperature"]),
                    soc_max=float(series_module_metadata["soc_max"]),
                    soc_min=float(series_module_metadata["soc_min"]),
                    crate_c=float(series_module_metadata["crate_c"]),
                    crate_d=float(series_module_metadata["crate_d"]),
                )
                result = conn.execute(stmt)
                conn.commit()
...
def import_module_data_into_buffer(df_configuration: pd.DataFrame, df_module_data: pd.DataFrame, module_id: str, engine):
    df_configuration.fillna(value=0, inplace=True)
    for index, row in df_configuration.iterrows():
        cell_id = "{}_{}".format(module_id, row["Name"])
        ...
        if row["Voltage column"]:
            df_cell_data["v"] = df_module_data[row["Voltage column"]]
        if row["Current column"]:
            df_cell_data["i"] = df_module_data[row["Current column"]]
        if row["Internal temperature column"]:
            df_cell_data["cell_temperature"] = df_module_data[row["Internal temperature column"]]
        if row["Ambient temperature column"]:
            df_cell_data["env_temperature"] = df_module_data[row["Ambient temperature column"]]
        ...
        if row["Type"] == "Module":
            set_module_status(module_id, "processing", engine)
        else:
            set_cell_status(cell_id, "processing", engine)


def process_module_data(module_id, engine):
    cell_metadata_table = Table("cell_metadata", metadata_obj, autoload_with=engine)
...
def import_all_module_data(df_module_metadata, base_filepath, engine):
    for index, row in df_module_metadata.iterrows():
        file_id = row["file_id"]
        ...
        if not os.path.exists(configuration_file) or len(data_files) != 1:
            ...
            logging.warning(f"Could not find both configuration and data files for module {module_id}")
            logging.warning(f"Skipping module...")
            continue
        df_configuration = excel_to_dataframe(configuration_file)
        xl_module_data = pd.ExcelFile(data_files[0])
        sheetname = None
        for sheet in xl_module_data.sheet_names:
            if "Channel" in sheet and "Chart" not in sheet:
                sheetname = sheet
        df_module_data = excel_to_dataframe(data_files[0], sheetname)
        logging.info(f"Importing data for module {module_id}")
        print("importing metadata")
        import_module_metadata(df_module_metadata, module_id, engine)
        import_module_cell_metadata(df_configuration, df_module_metadata, module_id, engine)
        print("importing into buffer")
        import_module_data_into_buffer(df_configuration, df_module_data, module_id, engine)
        print("processing")
        process_module_data(module_id, engine)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="module_data_import_agent",
        description="Imports module data into Battery Archive",
    )
    ...
    import_all_module_data(df_module_metadata, os.path.dirname(module_metadata_file), engine)